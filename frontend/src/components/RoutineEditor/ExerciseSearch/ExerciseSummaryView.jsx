...
const ExerciseSummaryView = ({
  ...
  const inputClasses =
    "w-full text-center px-4 py-3.5 rounded-[16px] bg-bg-primary border-none ring-1 ring-black/5 dark:ring-white/10 focus:ring-2 focus:ring-accent/50 outline-none transition-all font-bold text-text-primary [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none";
  const labelClasses =
            ...
            {stagedExercises.map((item) => {
              ...
              return (
                <div
                  ...
                  className="flex flex-col bg-black/5 dark:bg-white/5 rounded-[24px] ring-1 ring-black/5 dark:ring-white/10 transition-all duration-300 group overflow-hidden shadow-sm relative"
                >
                  ...
                  <div className="flex flex-col flex-1 p-5 pt-4">
                    ...
                    <div className="flex justify-between items-start gap-4 mb-4">
                      <div className="flex-1">
                        <h3 className="font-extrabold text-[20px] text-text-primary leading-tight line-clamp-2 pr-8">
                          {translatedName}
                        </h3>
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                          <span className="text-[10px] font-bold px-2 py-1 bg-black/5 dark:bg-white/10 text-text-secondary rounded-[8px] uppercase tracking-wide truncate max-w-[120px]">
                            {translatedMusclesList[0]}{" "}
                            {translatedMusclesList.length > 1 &&
                              `+${translatedMusclesList.length - 1}`}
                          </span>
                    ...
                    <div className="flex gap-2 sm:gap-3 mt-auto pt-4 border-t border-black/5 dark:border-white/10">
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          value={item.sets}
                          onChange={(e) =>
                            onUpdate(item.exercise.id, "sets", e.target.value)
                          }
