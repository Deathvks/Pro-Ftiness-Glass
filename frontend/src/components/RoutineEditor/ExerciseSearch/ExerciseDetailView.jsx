...
const ExerciseDetailView = ({
  ...
  const defaultDescription =
    ...
    t("exercise_ui:no_description_available", "No hay descripción disponible.");
  ...
  const isLimitReached =
    remainingUses === 0 ||
    (aiError && aiError.toLowerCase().includes("agotado"));
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      const errorMsg =
        data.error || error.message || "Error al conectar con la IA.";
      setAiError(errorMsg);
          ...
          <div className="w-full lg:w-7/12 xl:w-1/2 flex flex-col min-h-0">
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-black/5 dark:bg-white/5 p-5 rounded-[20px] ring-1 ring-black/5 dark:ring-white/10">
                <p className="text-[10px] sm:text-xs text-text-secondary uppercase tracking-wider font-bold mb-1">
                  {t("exercise_ui:muscle_group", "Grupo Muscular")}
                </p>
              ...
              <div className="bg-black/5 dark:bg-white/5 p-5 rounded-[20px] ring-1 ring-black/5 dark:ring-white/10">
                <p className="text-[10px] sm:text-xs text-text-secondary uppercase tracking-wider font-bold mb-1">
                  {t("exercise_ui:equipment", "Equipamiento")}
                </p>
                <p className="font-extrabold text-text-primary capitalize text-sm sm:text-base">
                  {translatedEquipment}
                </p>
            ...
            <div className="pb-6">
              <h3 className="text-xl font-bold mb-3 text-text-primary">
                {t("exercise_ui:description", "Descripción")}
              </h3>
            ...
            <div className="pt-6 border-t border-black/5 dark:border-white/10 space-y-5">
              <div className="flex justify-between items-center px-1">
                ...
                {remainingUses !== null && (
                  <div className="text-right">
                    <span
                      className={`text-[10px] sm:text-xs font-bold uppercase tracking-wider block ${isLimitReached ? "text-red-500" : "text-accent"}`}
                    >
                      Usos: {remainingUses}
                      {dailyLimit ? `/${dailyLimit}` : ""}
                    </span>
                    <span className="text-[9px] sm:text-[10px] text-text-muted block font-medium mt-0.5">
              ...
              {!aiExplanation && !isAiLoading && (
                <button
                  ...
                  className={`w-full p-4 rounded-[20px] flex items-center justify-center gap-2 transition-all active:scale-95 font-bold border-none ring-1 ${
                    isLimitReached
              ...
              {isAiLoading && (
                <div className="p-6 rounded-[24px] bg-black/5 dark:bg-white/5 ring-1 ring-black/5 dark:ring-white/10 flex flex-col items-center justify-center gap-4 py-10">
                  ...
                  <span className="text-sm font-bold text-text-secondary">
                    Analizando biomecánica del ejercicio...
                  </span>
                </div>
              )}
          ...
          >
            {isStaged ? (
              <>
                <Check size={24} />
                {t(
                  "exercise_ui:added_back_to_list",
                  "Añadido (Volver a la lista)",
                )}
              </>
            ) : (
              <>
