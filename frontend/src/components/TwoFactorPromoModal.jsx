...
const TwoFactorPromoModal = ({ onConfigure, onClose }) => {
  ...
  return <ModalPortal>
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 transition-all duration-300 ${isVisible ? 'opacity-100 backdrop-blur-sm' : 'opacity-0 backdrop-blur-none pointer-events-none'}`}>
      ...
      <div className={`relative w-full max-w-sm transform transition-all duration-300 ease-out ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        <GlassCard className="!bg-white dark:!bg-[--glass-bg] p-6 border-accent/20 shadow-[0_0_30px_rgba(var(--accent-rgb),0.15)]">
          <button
            onClick={handleClose}
            className="absolute top-4 right-4 p-1 text-text-secondary hover:text-text-primary dark:hover:text-white transition-colors rounded-full hover:bg-black/5 dark:hover:bg-white/10">
