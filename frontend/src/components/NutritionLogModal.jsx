...
const NutritionLogModal = ({ mealType, onClose, onSave, logToEdit, isLoading }) => {
  ...
  if (selectedDetailItem) {
    return <ModalPortal>
      <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-md animate-[fade-in_0.2s_ease-out]">
                <div className="relative w-full max-w-md overflow-hidden bg-bg-primary mt-auto sm:mt-0 rounded-t-[32px] rounded-b-none sm:rounded-[32px] ring-1 ring-black/5 dark:ring-white/10 shadow-2xl animate-[slide-up_0.3s_ease-out]" onClick={(e) => e.stopPropagation()}>
                    <FoodDetailView
  ...
  if (showScanner) {
    return <ModalPortal>
      <div className="fixed inset-0 bg-bg-primary z-[100] flex flex-col p-4 animate-[slide-in-up_0.3s]">
