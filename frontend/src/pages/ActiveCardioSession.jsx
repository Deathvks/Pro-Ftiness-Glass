...
import { calculateDistance, calculatePace, formatDistance } from '../utils/gpsUtils';
...
const ActiveCardioSession = ({ activityId: propActivityId, setView: propSetView }) => {
    ...
    const mapConfig = useMemo(() => {
        // Detectar si el sistema es oscuro si el tema es 'system'
        ...
        if (effectiveTheme === 'light') {
            return {
                ...
                opacity: 1,
                bgColor: '#f7fafc', // Fondo claro web
                className: '',
                gradientFrom: 'from-bg-primary/80' // Usa variable CSS
            };
        } else if (effectiveTheme === 'oled') {
            return {
                url: "https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",
                opacity: 1,
                bgColor: '#000000', // Negro puro
                className: '',
                gradientFrom: 'from-black/80'
            };
        } else {
            // TEMA DARK (Estándar): Usamos el color #0c111b
            // Bajamos opacidad al 0.5 para que el fondo azul oscuro se note mucho más
            return {
                url: "https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",
                opacity: 0.5,
                bgColor: DARK_THEME_COLOR,
                className: 'map-tiles-dark-adjusted', // Filtro CSS abajo
                gradientFrom: 'from-bg-primary/80' // Usará el color #0c111b definido en CSS
            };
        }
    }, [theme]);
    ...
    const startSession = (isResuming = false) => {
        ...
        return (
            <div className="min-h-screen bg-bg-primary flex flex-col items-center justify-center p-6 text-center animate-fade-in">
                ...
                <p className="text-text-secondary mb-8">
                    ...
                    Por favor, actívala en los ajustes de tu navegador.
                </p>
    ...
    return (
        <div className="relative w-full h-screen bg-bg-primary flex flex-col overflow-hidden">
            {/* Estilos para ajustar el mapa en modo Dark (no OLED) */}
            <style>
                {`
            ...
            {/* Fondo del contenedor del mapa controlado por mapConfig.bgColor */}
            <div
            ...
            >
                {position ? (
                    <MapContainer
                        center={position}
                        zoom={16}
                        // IMPORTANTE: background transparent para que se vea el div padre
